...
### 🚀 一键部署脚本（✨ 推荐方式，Podman 原生）

一键部署脚本会完成以下步骤：

- 检查 Linux + rootless Podman 运行环境
- 创建部署目录与状态文件
- 生成安全的管理员令牌和数据库密码
- 创建 Podman pod，并启动 PostgreSQL / Redis / App / 可选 Caddy
- 等待健康检查并输出访问地址
...
- ⚠️ 请妥善保存脚本输出的 **管理员令牌**（Admin Token）
- ⚠️ 官方 Podman 路径仅支持 Linux
- ⚠️ 若启用内置 Caddy，默认使用高位端口 `8080/8443`，不承诺 `80/443 + HTTP-01` 自动签发
...
> 💡 **提示**：
> - 如需修改 app 端口，使用 `./scripts/deploy.sh --port <port>`。
> - 如需启用高位端口 Caddy，使用 `./scripts/deploy.sh --enable-caddy`。
...
### 🦭 Podman（✨ 推荐方式，rootless）

Podman 是**首选部署方式**。部署脚本会在 Linux 上创建 rootless pod，并用 bind mount 保存 PostgreSQL / Redis / Caddy 数据。
...
4. 可选 Caddy：
   ...
   ```
   若使用 `--domain`，脚本会生成高位端口 HTTPS 配置；正式 TLS 建议交给外部网关或 DNS-01。
...
3. 常用命令：
   ...
   - `make app`：构建并运行 Podman app 容器（默认端口 `23000`）
...
1. **数据库连接失败怎么办？**
   - 确认 `DSN` 格式与凭据无误；容器内建议使用 `127.0.0.1:5432` 或实际暴露端口。
   - 查看 `podman pod ps` 或本地 PostgreSQL 状态，必要时通过 `make db-shell` 诊断。
...
6. **rootless Podman 目录权限报错？**
   - PostgreSQL 等数据目录需要使用容器内部 UID/GID，而不是宿主机 UID/GID。
   - 推荐通过脚本自动处理，或手动执行 `podman unshare chown -R 999:999 ~/.local/share/claude-code-hub/data/postgres`。
   - 若宿主机启用了 SELinux，挂载目录会自动追加 `:Z`；共享目录才使用 `:z`。
